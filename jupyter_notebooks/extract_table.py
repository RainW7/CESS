...
# list(f['parameters_name'][:]).index(b'z_{mean}')
# list(f['parameters_name'][:]).index(b'z_{median}')
# list(f['parameters_name'][:]).index(b'z_{MAP}')
# list(f['parameters_name'][:]).index(b'z_{MAL}')

# list(f['parameters_name'][:]).index(b'log(M*)[0,1]_{mean}')
# list(f['parameters_name'][:]).index(b'log(M*)[0,1]_{median}')
# list(f['parameters_name'][:]).index(b'log(M*)[0,1]_{MAP}')
# list(f['parameters_name'][:]).index(b'log(M*)[0,1]_{MAL}')

# list(f['parameters_name'][:]).index(b'log(SFR_100Myr/[M_{sun}/yr])[0,1]_{mean}')
# list(f['parameters_name'][:]).index(b'log(SFR_100Myr/[M_{sun}/yr])[0,1]_{median}')
# list(f['parameters_name'][:]).index(b'log(SFR_100Myr/[M_{sun}/yr])[0,1]_{MAP}')
# list(f['parameters_name'][:]).index(b'log(SFR_100Myr/[M_{sun}/yr])[0,1]_{MAL}')

# list(f['parameters_name'][:]).index(b'log(H1_6562.81)[0,1]_{mean}')
# list(f['parameters_name'][:]).index(b'log(H1_6562.81)[0,1]_{median}')
# list(f['parameters_name'][:]).index(b'log(H1_6562.81)[0,1]_{MAP}')
# list(f['parameters_name'][:]).index(b'log(H1_6562.81)[0,1]_{MAL}')

# list(f['parameters_name'][:]).index(b'E(B-V)')

# list(f['parameters_name'][:]).index(b'log(H1_4861.33)[0,1]_{mean}')
# list(f['parameters_name'][:]).index(b'log(H1_4861.33)[0,1]_{median}')
# list(f['parameters_name'][:]).index(b'log(H1_4861.33)[0,1]_{MAP}')
# list(f['parameters_name'][:]).index(b'log(H1_4861.33)[0,1]_{MAL}')

extract_param = ['z_{mean}','z_{median}','z_{MAP}','z_{MAL}',
                 'log(M*)[0,1]_{mean}', 'log(M*)[0,1]_{median}', 'log(M*)[0,1]_{MAP}', 'log(M*)[0,1]_{MAL}', 
                 'log(SFR_100Myr/[M_{sun}/yr])[0,1]_{mean}', 'log(SFR_100Myr/[M_{sun}/yr])[0,1]_{median}', 'log(SFR_100Myr/[M_{sun}/yr])[0,1]_{MAP}','log(SFR_100Myr/[M_{sun}/yr])[0,1]_{MAL}',
                 'log(H1_6562.81)[0,1]_{mean}', 'log(H1_6562.81)[0,1]_{median}', 'log(H1_6562.81)[0,1]_{MAL}', 'log(H1_6562.81)[0,1]_{MAP}', 
                 'log(H1_4861.33)[0,1]_{mean}', 'log(H1_4861.33)[0,1]_{median}', 'log(H1_4861.33)[0,1]_{MAL}', 'log(H1_4861.33)[0,1]_{MAP}',
                 'E(B-V)']

def create_fits_table(file_paths):
    k = 0
    for i in range(len(extract_param)):
        
        id_list = []
        data_list = []
        print('start {0}'.format(extract_param[i]))
        # 对所有的hdf5文件循环
        for file_path in file_paths:
            with h5py.File(file_path, 'r') as f:
                # 读取每个hdf5文件的id和要提取的data
                idx = list(f['parameters_name'][:]).index(extract_param[i])
                id_data =  f['ID'][:]
                data = f['parameters'][:,idx]
                id_list.append(id_data)
                data_list.append(data)
                # 储存所有的hdf5文件的id和data
                k+=1
            print(k)
        print('end {0}'.format(extract_param[i]))
        id_array = np.concatenate(id_list)
        data_array = np.concatenate(data_list)
        table = Table([id_array, data_array], names=('ID', extract_param[i]))
        table.write('gu_'+extract_param[i]+'.fits', overwrite=True)
    print('gu_'+extract_param[i]+'.fits created')

hdf5names = ['seedcat2_0420_0_DECaLS_0csp_sfh200_bc2003_hr_stelib_chab_neb_300r_i0100_2dal8_10.hdf5',
