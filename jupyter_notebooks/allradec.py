...
def read_hdf5_file(file_path):
    with h5py.File(file_path, 'r') as f:
        id_data = f['ID'][:]
        ra_data = f['parameters'][:,14]
        decl_data = f['parameters'][:,15]
    return id_data, ra_data, decl_data
...
def create_fits_table(file_paths):
    ...
    for file_path in file_paths:
        id_data, ra_data, decl_data = read_hdf5_file(file_path)
        id_list.append(id_data)
        ra_list.append(ra_data)
        decl_list.append(decl_data)
        file_name_list += [file_path.split("_")[2] + "_" + file_path.split("_")[3]] * len(id_data)
        i = i+1
        print(i)
    id_array = np.concatenate(id_list)
    ra_array = np.concatenate(ra_list)
    decl_array = np.concatenate(decl_list)
    file_name_array = np.array(file_name_list)
    
    table = Table([id_array, ra_array, decl_array, file_name_array], names=('ID', 'ra', 'dec', 'filename'))
    table.write('radec_info.fits', overwrite=True)
