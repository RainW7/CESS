...
'''
@Env     		:   grizli
@File    		:   ~/emulator/emulator_v0.7/main.py
@Time    		:   2023/06/21 09:10:21
@Author  		:   Run Wen
@Version 		:   0.7
@Contact 		:   wenrun@pmo.ac.cn
@Description	:   The main file of emulator containing functions for convolving and adding noise


Change log:
1.  Create a json file for input parameters which includes CSST slitless intrumental parameters to be updated in future. 
2.  Update the noise calculation methods, following sls_1d_spec by Xin Zhang, now is calculated by a extracted height length for background counts.
3.  Update the length of slitless spectra of CSST, now strictly following Xingcheng Zhou et al. (2021), which shows 
    GU R ~ 241@3375Å, corresponding to , GV R ~ 263@5250Å, GI R ~ 270@8100Å
4. Using json to load the CSST intrumental parameters
...
guRes = emulator_parameters['guRes'] #resolution of grism
gvRes = emulator_parameters['gvRes']
giRes = emulator_parameters['giRes']
arcsecperpix = emulator_parameters['arcsecperpix']# arcsec/pixel
dc_value = emulator_parameters['dc_value']

GUwavemin = emulator_parameters['GUwavemin']
GUwavemax = emulator_parameters['GUwavemax']
GVwavemin = emulator_parameters['GVwavemin']
GVwavemax = emulator_parameters['GVwavemax']
GIwavemin = emulator_parameters['GIwavemin']
GIwavemax = emulator_parameters['GIwavemax']
...
class emulator(object):
    ...
    def sls_wave_spec(self,highres_wave,highres_flux,redshift,grism):
        ...
        # 根据光谱分辨单元的大小计算波长数组中元素的数量
        N = int((end_wl - start_wl) / delta_lambda) + 1
        # 根据波长范围创建一个满足该波段分辨率的光谱分辨单元数组
        ...
        # 根据波长范围截断grism波段的原始高分辨率光谱的波长和流量数组
        grism_wave_list = []
        grism_flux_list = []
        for i in (range(len(wave_obs))):
        ...
        return spectral_unit_wavelength, spectral_unit_flux
    

    def kernels_convolve(self,highres_wave,highres_flux,redshift,grism):
        """ 
        Convolve CSST intrinsic spectra with CSST grsim resolution and expanding grid points.
        ...
        wave, flux = self.sls_wave_spec(highres_wave,highres_flux,redshift,grism)
    ...
    def wave_err(self,flux,width):
        """ 
        Add a wavelength offset in slitless spectrum according to the width profile. 
        1 percent error is given as the upper limit of width profile is 9 (4 pixel of R_EE80 <= 0.3'' PSF for CSST).
        ...
        return:
        ------
            flux_new - flux of the slitless spectrum with wavelength extraction errors within 4 pixels - [array]
        """
        if len(width) > 10:
            wave_err_distribution = utils.gaussian(4,2)
        elif len(width) <= 9:
            wave_err_distribution = width/width.sum()
        x_center_pos = int(wave_err_distribution.shape[0]/2)
        wave_off = np.random.choice(np.arange(-x_center_pos,x_center_pos+1,1), p=wave_err_distribution)
        if wave_off == 0:
            flux_new = flux
        if wave_off > 0:
            flux_new = np.hstack((flux[:wave_off]*wave_err_distribution[:wave_off],flux))[:-wave_off]
        if wave_off < 0:
            flux_new = np.hstack((flux,flux[:-wave_off]*wave_err_distribution[:-wave_off]))[-wave_off:]
    ...
    def add_noise(self, ra, dec, wave, fnu, bkgwave, bkgflux, tp, height, width):
        """
        ...
        # adding a 1% precision error (about 4 pixel offset) in wavelength grid, affect on the flux grid
        wave_off, flux = self.wave_err(flux_, width)
        throughput = tp
        ...
        totalnoise = bkg_counts_per_column + np.random.poisson(elec_per_column + dc_noise_column) + read_noise_along_dis
        ...
        # if elec_with_noise < 0, set elec_with_noise = 0
        elec_with_noise = [x if x >= 0 else 0 for x in elec_with_noise] 
        # converting electrons into photons and then fnu
        photon_with_noise = elec_with_noise / throughput
        fnu_with_noise = utils.fphot2fnu(wave, photon_with_noise / (colarea*expt*expnum*dlambda))/0.7
        # calculating flux error
        elec_err = elec_per_column + bkg_counts_per_column + dc_noise_column + 5**2 * len(height) * expnum
        phot_err = np.sqrt(elec_err) / throughput
        fnu_error = utils.fphot2fnu(wave, phot_err/(colarea*expt*expnum*dlambda))
        # calculating signal-to-noise ratio
        snr = elec_with_noise/np.sqrt(elec_per_column + bkg_counts_per_column + dc_noise_column + expnum*(len(height))*5**2)
        snr_mean = np.mean(snr)

        return elec_per_column, fnu_with_noise, fnu_error, rms_in_e, snr, snr_mean, wave_off
