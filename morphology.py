...
'''
@Env     		:   grizli
@File    		:   ~/emulator/emulator_v0.7/morphology.py
@Time    		:   2023/07/03 11:05:01
@Author  		:   Run Wen
@Version 		:   0.7
...
Change logs:
1.  The signum of PA is now considered.
2.  Update 2-D profile modeling packages from astropy.modeling to galsim since astropy.modeling cannot constrain the total flux.
3.  Update the extraction box selection method, following sls_1d_spec, I decide the width of extraction box as a flux ratio
    compared with the total flux of the spectrum in the spatial direction, the ratio varies with sersic index due to the feature 
    of galsim, which creates extremely large image when n is high. 
4.  Update the 2-D profile parameters fitting process, now strictly following the van der Wel et al. (2014) mass-size relation and
    GEMS catalog n-mag relation (with posibility).
5.  Update the extraction box selection method, following sls_1d_spec, I decide the width of extraction box as a flux ratio
    compared with the total flux of the spectrum in the spatial direction. 
'''
...
gems_cat = fits.open('/Users/rain/emulator/gems_cat.fits') # for my mac
# gems_cat = fits.open('/home/runwen/emulator_v0.7/gems_cat.fits') # for starburst server
