...
@File    		:   ~/emulator/emulator_v0.8/run_spec_han_240605.py
@Time    		:   2024/06/04 16:19:26
@Author  		:   Run Wen
@Version 		:   0.84
...
desipath = '/Users/rain/emulator/emulator_v0.84/'
...
def task(desipath,hdf5filename):
    ...
    guwave, gu_ujy, mask, mask_nan = emulator.kernels_convolve_new(f['best_fit']['wavelength_rest'][:]*1e4,
                                               f['best_fit']['spec_csp_sfh201_bc2003_hr_stelib_kroup_neb_300r_i0100_2dal8[0,1]'], 
                                               f['best_fit']['z'], 
                                               'GU', gu_ree80)
    gvwave, gv_ujy, mask, mask_nan = emulator.kernels_convolve_new(f['best_fit']['wavelength_rest'][:]*1e4,
                                               f['best_fit']['spec_csp_sfh201_bc2003_hr_stelib_kroup_neb_300r_i0100_2dal8[0,1]'], 
                                               f['best_fit']['z'], 
                                               'GV', gv_ree80)
    giwave, gi_ujy, mask, mask_nan = emulator.kernels_convolve_new(f['best_fit']['wavelength_rest'][:]*1e4,
                                               f['best_fit']['spec_csp_sfh201_bc2003_hr_stelib_kroup_neb_300r_i0100_2dal8[0,1]'], 
                                               f['best_fit']['z'], 
                                               'GI', gi_ree80)
    ...
    raall = f['parameters'][:,14]
    decall = f['parameters'][:,15]
    m_starall = f['parameters'][:,115]
    zall = f['parameters'][:,31]
    maggall = f['parameters'][:,4]
    magrall = f['parameters'][:,6]
    magzall = f['parameters'][:,8]
    ...
    for i in (range(len(f['ID'][:]))):
        ...
        if z in zall[mask]:
            ...
            # get convolved spectra in AA and erg/s/cm^2/Hz for the photometric estimation
            phot_wave, phot_flux = emulator.phot_convolve(f['best_fit']['wavelength_rest'][:]*1e4*(1+f['best_fit']['z'][i]),f['best_fit']['spec_csp_sfh201_bc2003_hr_stelib_kroup_neb_300r_i0100_2dal8[0,1]'][i]*1e-29)
            ...
            gu_elec_per_column_single, gu_fnu_with_noise_single, gu_fnu_error_single, gu_rms_in_e_single, gu_snr_single, gu_snr_mean_single, gu_wave_off_single, gu_spec_mask_single, gu_snr_mask_single = emulator.add_noise(ra,dec,guwave,gu_fnu[i],guwave,gu_bkg_fnu,gutp,height,width,'GU')
            gv_elec_per_column_single, gv_fnu_with_noise_single, gv_fnu_error_single, gv_rms_in_e_single, gv_snr_single, gv_snr_mean_single, gv_wave_off_single, gv_spec_mask_single, gv_snr_mask_single = emulator.add_noise(ra,dec,gvwave,gv_fnu[i],gvwave,gv_bkg_fnu,gvtp,height,width,'GV')
            gi_elec_per_column_single, gi_fnu_with_noise_single, gi_fnu_error_single, gi_rms_in_e_single, gi_snr_single, gi_snr_mean_single, gi_wave_off_single, gi_spec_mask_single, gi_snr_mask_single = emulator.add_noise(ra,dec,giwave,gi_fnu[i],giwave,gi_bkg_fnu,gitp,height,width,'GI')
    ...
    # saveing data 
    parameters_desi = np.hstack([raall.reshape((len(f['ID'][:]),1)),decall.reshape((len(f['ID'][:]),1)),zall.reshape((len(f['ID'][:]),1)),
    ...
    with h5py.File("CSST_grism"+'_'+str(len(f['ID'][:]))+'_'+hdf5filename, "w") as file:
        ...
        # spec mask
        dataset21 = group_gu.create_dataset('spec_mask', data = gu_spec_mask)
        dataset22 = group_gv.create_dataset('spec_mask', data = gv_spec_mask)
        dataset23 = group_gi.create_dataset('spec_mask', data = gi_spec_mask)
        # snr mask 
        dataset24 = group_gu.create_dataset('snr_mask', data = gu_snr_mask)
        dataset25 = group_gv.create_dataset('snr_mask', data = gv_snr_mask)
        dataset26 = group_gi.create_dataset('snr_mask', data = gi_snr_mask)
...
hdf5filenames = [
# 'seedcat2_0_DECaLS_0csp_sfh201_bc2003_hr_stelib_kroup_neb_300r_i0100_2dal8_10_inoise4.hdf5',
'seedcat2_0_MzLS_0csp_sfh201_bc2003_hr_stelib_kroup_neb_300r_i0100_2dal8_10_inoise4.hdf5'
]
