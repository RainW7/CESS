...
@Time    		:   2024/09/30 12:24:22
@Author  		:   Run Wen
@Version 		:   0.8.5
@Contact 		:   wenrun@pmo.ac.cn
@Description	:   create demo fig for simulated CSST slitless spectrum
'''
...
flux_obs = mAB2uJy(hdf5['parameters_desi'][idx,[i for i in [3,4,5]]])
...
redshift = (1+hdf5['parameters_desi'][idx,2])
...
plt.suptitle('File: {0}, \n index = {1}, DESI_ID = {2}, (RA, DEC) = ({3}, {4}), z = {5}\n'
            ...
            .format(hdf5.filename.split('/')[-1], idx+1, hdf5['ID'][idx], hdf5['parameters_desi'][idx,0], hdf5['parameters_desi'][idx,1], format(redshift-1,'.2f'),
                    hdf5['parameters_desi'][idx,3], hdf5['parameters_desi'][idx,4], hdf5['parameters_desi'][idx,5],
                    format(hdf5['parameters_desi'][idx,6],'.2f'), format(hdf5['parameters_desi'][idx,7],'.2f'), format(hdf5['parameters_desi'][idx,8],'.2f'), format(hdf5['parameters_desi'][idx,9],'.2f'),format(hdf5['parameters_desi'][idx,10],'.1f'),
                    format(hdf5['parameters_grism'][idx,3],'.2f'),format(hdf5['parameters_grism'][idx,4],'.2f'),format(hdf5['parameters_grism'][idx,5],'.2f')),fontsize=9)
plt.tight_layout()
