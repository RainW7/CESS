...
@File    		:   ~/emulator/CESS_v0.8.5/main.py
@Time    		:   2024/11/04 16:12:48
@Author  		:   Run Wen
@Version 		:   0.8.5
...
5.  Add photomentric simulation for 7 CSST photomentric bands.
'''
...
class emulator(object):
    ...
    def sls_wave_spec(self, highres_wave, highres_flux, redshift, grism):
        ...
        mask1 = (redshift[:] >= 0.00000001) & (redshift[:] <= 10) # there are some problems in some DESI phot catalog, see problem 1 in errors.log
        mask2 = np.nanmax( abs( np.log10( abs( highres_flux ))), axis=1) <= 100 # there are some problems in some DESI phot catalog, see problem 2 in errors.log, np.log10(1e10) = 10, max value = 10^10
        mask_nan1 = (redshift[:] < 0.00000001) | (redshift[:] > 10)
        mask_nan2 = np.nanmax( abs( np.log10( abs( highres_flux ))), axis=1) > 100 
        mask = mask1 & mask2
        mask_nan = mask_nan1 | mask_nan2
        sigma_temp = spectral_unit_wavelength/(highres_wave[0]/(highres_wave[1]-highres_wave[0]))/2.35482
        sigma_inst = delta_lambda/2.35482
        sigma_temp[sigma_temp>sigma_inst] = sigma_inst
        sigma = np.ones_like(spectral_unit_wavelength) * np.sqrt(sigma_inst**2-sigma_temp**2)

        return spectral_unit_wavelength, sigma, mask, mask_nan

    def kernels_convolve(self, highres_wave, highres_flux, redshift, grism, fwhm):
        """ 
        ...
        spectral_unit_wavelength, sigma, mask, mask_nan = self.sls_wave_spec(highres_wave,highres_flux,redshift,grism)
        ...
        # generate the corresponding wave and flux array for three grism bands
        grism_wave_list = []
        grism_flux_list = []
        for i in (range(len(wave_obs))):
        ...
        # convolve second
        fnew_total = np.zeros([len(spectral_unit_flux),len(spectral_unit_wavelength)])
        for i in range(len(spectral_unit_flux)):
            fnew_total[i]  = self.varsmooth(spectral_unit_wavelength, spectral_unit_flux[i], sigma)
        fnew_total_arr = np.array(fnew_total)

        # interpolate third
        flux_func = interp1d(spectral_unit_wavelength, fnew_total_arr)
        new_wave = np.linspace(min_wl, max_wl, round(len(spectral_unit_wavelength) * fwhm/arcsecperpix))
    ...
    def add_noise(self, ra, dec, wave, fnu, bkgwave, bkgflux, tp, height, width, grism, apply_morphology_effect, apply_wavelength_calibration_error):
        """
        ...
        ----------
            ...
            grism - three CSST grism band - [str]
            apply_morphology_effect - whether to apply morphology effect or not - [bool]
            apply_wavelength_calibration_error - whether to apply wavelength calibration error or not - [bool]
        ...
        if apply_morphology_effect==True:
            flux_ = self.lamshift_width(fnu,width)
        elif apply_morphology_effect==False:
            flux_ = fnu
        ...
        elif apply_wavelength_calibration_error==False:
            wave_off = 0
            flux = flux_
    ...
    def add_noise_zouhu(self, ra, dec, wave, fnu, bkgwave, bkgflux, tp, height, width, grism, apply_morphology_effect, apply_wavelength_calibration_error):
